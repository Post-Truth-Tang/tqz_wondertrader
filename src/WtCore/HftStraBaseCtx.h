...
public:
	...
	virtual void tqz_writeLog(string logFilename, const string logContent) override;

	virtual double stra_get_position(const char* stdCode) override;
	virtual double tqz_getLongPosition(const char* stdCode) override;
	virtual double tqz_getShortPosition(const char* stdCode) override;
	...
	virtual uint32_t tqz_getTradingDate() override;
...
protected:
	...
	inline void	log_close(const char* stdCode, bool isLong, uint64_t openTime, double openpx, uint64_t closeTime, double closepx, double qty, double profit, double maxprofit, double maxloss, double totalprofit, const char* enterTag, const char* exitTag);
...
protected:
	...
	WTSArray* _uncancelOrders; // tqz add.
