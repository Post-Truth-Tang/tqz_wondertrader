...
bool HftStraBaseCtx::stra_cancel(uint32_t localid) {
	...
	if (NULL != _trader) {
		...
		this->stra_log_text("[HftStraBaseCtx::stra_cancel], NULL != _trader localid: %d", localid);
		return false;
	}
}
...
void HftStraBaseCtx::tqz_writeLog(string logFilename, const string logContent) {

	string basic_logger_name = "Logs/strategies/" + logFilename + ".log";
	auto strategies_logger = rotating_logger_mt("tqz_hft_strategies log", basic_logger_name.c_str(), 1048576 * 5, 3);
	
	strategies_logger->info(logContent.c_str());
	strategies_logger->flush();
	spdlog::drop_all();
}
...
double HftStraBaseCtx::tqz_getLongPosition(const char* stdCode) {
	return this->_trader->getPosition(stdCode, 1);
}

double HftStraBaseCtx::tqz_getShortPosition(const char* stdCode) {
	return this->_trader->getPosition(stdCode, 2);
}
...
uint32_t HftStraBaseCtx::tqz_getTradingDate() {
	return this->_engine->get_trading_date();
}
...
uint32_t HftStraBaseCtx::tqz_getCancelCounts(const char* stdCode) {
	return this->_trader->tqz_getCancelCounts(stdCode);
}
