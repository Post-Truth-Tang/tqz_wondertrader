...
double TraderAdapter::getPosition(const char* stdCode, int32_t flag /* = 3 */) // get net position.
{
...
bool TraderAdapter::checkCancelLimits(const char* stdCode) {
	...
	if (it != _cancel_time_cache.end()) {
		...
		if (cnt >= riskPara->_cancel_times_boundary) {
			...
			uint32_t sIdx = uint32_t(tit - cache.begin());
...
OrderIDs TraderAdapter::buy(const char* stdCode, double price, double qty) {
	...
	for (auto it = ruleGP.begin(); it != ruleGP.end(); it++)
	{
		...
		if(curRule._atype == AT_Open) {
			...
			//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of buying {} of quantity {} triggered: Opening long of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
		}
		else if(curRule._atype == AT_CloseToday)
		{
			...
			if (decimal::gt(maxQty, 0))
			{
				...
				if (commInfo->getCoverMode() == CM_CoverToday)
				{
					...
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO,  fmt::format("[{}] Signal of buying {} of quantity {} triggered: Closing new short of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				}
				else
				{
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}]{} 买入数量{}信号执行: 平空数量{}", _id.c_str(), stdCode, qty, maxQty).c_str());
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of buying {} of quantity {} triggered: Closing short of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				}
			}
		...
		else if (curRule._atype == AT_CloseYestoday)
		{
			...
			if (decimal::gt(maxQty, 0))
			{
				...
				if (commInfo->getCoverMode() == CM_CoverToday) {
					...
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of buying {} of quantity {} triggered: Closing old short of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				} else {
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}]{} 买入数量{}信号执行: 平空数量{}", _id.c_str(), stdCode, qty, maxQty).c_str());
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of buying {} of quantity {} triggered: Closing short of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				}
...
OrderIDs TraderAdapter::sell(const char* stdCode, double price, double qty) {
	...
	for (auto it = ruleGP.begin(); it != ruleGP.end(); it++) {
		...
		if (curRule._atype == AT_Open) {
			...
			//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of selling {} of quantity {} triggered: Opening short of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
		}
		else if (curRule._atype == AT_CloseToday)
		{
			double maxQty = 0;
			...
			if(decimal::gt(maxQty, 0)) {
				...
				if (commInfo->getCoverMode() == CM_CoverToday) {
					...
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of selling {} of quantity {} triggered: Closing new long of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				} else {
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}]{} 卖出数量{}信号执行: 平多{}", _id.c_str(), stdCode, qty, maxQty).c_str());
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of selling {} of quantity {} triggered: Closing long of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				}
			}
			...
			if(decimal::gt(maxQty, 0))
			{
				...
				if (commInfo->getCoverMode() == CM_CoverToday)
				{
					...
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Signal of selling {} of quantity {} triggered: Closing old long of quantity {}", _id.c_str(), stdCode, qty, maxQty).c_str());
				}
				else
				{
					//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}]{} 卖出数量{}信号执行: 平多{}", _id.c_str(), stdCode, qty, maxQty).c_str());
...
bool TraderAdapter::doCancel(WTSOrderInfo* ordInfo) {
	if (ordInfo == NULL || !ordInfo->isAlive()) {
		if (ordInfo == NULL)
			std::cout << "[TraderAdapter::doCancel] 2.1, ordInfo == NULL" << std::endl << std::endl;
		if (!ordInfo->isAlive())
			std::cout << "[TraderAdapter::doCancel] 2.2, !ordInfo->isAlive()" << std::endl << std::endl;
...
bool TraderAdapter::cancel(uint32_t localid) {
	if (_orders == NULL || _orders->size() == 0) {
		if (_orders == NULL) {
			//std::cout << "[TraderAdapter::cancel] _orders == NULL, localid: " << localid << std::endl << std::endl;
		}
		if (_orders->size() == 0) {
			//std::cout << "[TraderAdapter::cancel] _orders->size() == 0, localid: " << localid << std::endl << std::endl;
		}
		return false;
	}
	...
	bool bRet = false;
	if (ordInfo == NULL) {
		//std::cout << "[TraderAdapter::cancel] ordInfo == NULL, localid: " << localid << std::endl << std::endl;
		return false;
	} else {
		bRet = doCancel(ordInfo);
	}
...
void TraderAdapter::onRspOrders(const WTSArray* ayOrders) {
	...
	if (ayOrders) {
		...
		for (auto it = ayOrders->begin(); it != ayOrders->end(); it++) {
			...
			// cancel uncanceled order when re connect; tqz add 
			if (orderInfo->getOrderState() == WOS_NotTraded_NotQueuing || orderInfo->getOrderState() == WOS_NotTraded_Queuing || orderInfo->getOrderState() == WOS_PartTraded_NotQueuing || orderInfo->getOrderState() == WOS_PartTraded_Queuing)
				this->doCancel(orderInfo);
...
void TraderAdapter::onPushOrder(WTSOrderInfo* orderInfo) {
	if (orderInfo == NULL)
		return;
	//std::cout << "[TraderAdapter::onPushOrder] instrument: " << orderInfo->getCode() << ", entrustid: " << orderInfo->getEntrustID() << ", orderid: " << orderInfo->getOrderID() << ", usertag: " << orderInfo->getUserTag() << ", state: " << stateToName(orderInfo->getOrderState()) << std::endl << std::endl;
	...
	//WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,"[%s TraderAdapter::onPushOrder] Order notified, instrument: %s, entrustid: %s, orderid: %s, usertag: %s, state: %s", _id.c_str(), orderInfo->getCode(), orderInfo->getEntrustID(), orderInfo->getOrderID(), orderInfo->getUserTag(), stateToName(orderInfo->getOrderState()));
	...
	//先看看是不是wt发出去的单子
	if (StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern)) {
		//std::cout << "[TraderAdapter::onPushOrder] 14 instrument: " << orderInfo->getCode() << ", entrustid: " << orderInfo->getEntrustID() << ", orderid: " << orderInfo->getOrderID() << ", usertag: " << orderInfo->getUserTag() << ", state: " << stateToName(orderInfo->getOrderState()) << std::endl << std::endl;
		char* userTag = (char*)orderInfo->getUserTag();
		userTag += _order_pattern.size() + 1;
...
uint32_t TraderAdapter::tqz_getCancelCounts(const char* stdCode) {

	WTSTradeStateInfo* statInfo = (WTSTradeStateInfo*)_stat_map->get(stdCode);
	if (NULL == statInfo) {
		return 0;
	} else {
		return statInfo->statInfo().b_cancels + statInfo->statInfo().s_cancels;
	}
}
...
void TraderAdapter::onPushTrade(WTSTradeInfo* tradeRecord) {
	...
	//WTSLogger::log_dyn_raw("trader", _id.c_str(), LL_INFO, fmt::format("[{}] Trade notified, instrument: {}, usertag: {}, trdqty: {}, trdprice: {}", _id.c_str(), stdCode.c_str(), tradeRecord->getUserTag(), tradeRecord->getVolume(), tradeRecord->getPrice()).c_str());
...
void TraderAdapter::onTraderError(WTSError* err) {
	if(err)
		WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,"[%s TraderAdapter::onTraderError] Error of trading channel occured: %s", _id.c_str(), err->getMessage());
