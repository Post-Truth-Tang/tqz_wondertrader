...
int TraderCTP::orderInsert(WTSEntrust* entrust) {
	...
	int iResult = m_pUserAPI->ReqOrderInsert(&req, genRequestID()); // 如果 iResult 不是零, 可能被流控
	if (iResult != 0)
...
int TraderCTP::orderAction(WTSEntrustAction* action) {
	...
	if (!extractEntrustID(action->getEntrustID(), frontid, sessionid, orderref)) {
		m_sink->handleTraderLog(LL_INFO, "[TraderCTP] TraderCTP::orderAction, action->getEntrustID(): %s, frontid: %d, sessionid: %d, orderref: %d", action->getEntrustID(), frontid, sessionid, orderref);
		return -1;
	}
...
void TraderCTP::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (!IsErrorRspInfo(pRspInfo))
	{
		...
		m_sink->handleTraderLog(LL_INFO, "[TraderCTP][%s-%s] Login succeed, AppID: %s, Sessionid: %u, login time: %s...", m_strBroker.c_str(), m_strUser.c_str(), m_strAppID.c_str(), m_sessionID, pRspUserLogin->LoginTime);
